...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if model_base is not None:
            ...
            from peft import PeftModel
            tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            model = AutoModelForCausalLM.from_pretrained(model_base, **kwargs, low_cpu_mem_usage=False, device_map=device_map)
    ...
    if 'llava' in model_name.lower():
        ...
        if not vision_tower.is_loaded:
            # 핵심: vision에는 LLM용 device_map(dict) 넘기지 말기!
            # (meta 텐서 경고/assign 이슈를 피하는 쪽으로)
            try:
                vision_tower.load_model(device_map=None)
            except TypeError:
                # load_model 시그니처가 device_map을 안 받는 버전이면 그냥 호출
